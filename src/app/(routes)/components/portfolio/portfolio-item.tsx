  ...
  <div className={`shadow-xl shadow-black/30 rounded-xl border-4 border-tron-sky-800 mb-2 sm:mb-7 ${className}`}>
    ...
    <div className="flex justify-between bg-gradient-to-b from-tron-sky-800 from-70% to-tron-sky-700 border-t-8 border-black/25 rounded-b-md h-12 px-2 pb-1 items-center">
      ...
      <div className="flex gap-x-2">
        <Link href={websiteUrl} target="_blank"><Button><FaGlobe className="w-3 h-3 sm:w-5 sm:h-5 text-white" /></Button></Link>
        <Button><FaEye className="w-3 h-3 sm:w-5 sm:h-5 text-white" /></Button>
        <Link href={githubUrl} target="_blank"><Button><FaGithub className="w-3 h-3 sm:w-5 sm:h-5 text-white" /></Button></Link>
      </div>
    </div>
  </div>
)
